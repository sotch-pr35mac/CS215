...
A merge sort can be implemented in a variety of languages using the pseudocode provided below. 
\newline
\textbf{Merge Sort Pre-Condition}: A is a non-empty array of comparable data with a natural order.
\newline
\textbf{Merge Sort Post-Condition}: A' is a permutation of A (containing all the same elements) in stricly non-decreasing order.
\begin{algorithm}
